...
// PHP backend lives at the same origin under /api
const API_URL = "";
...
const API = {
    async call(endpoint, options = {}) {
        try {
            ...
            // Silence the "Not authenticated" toast during initial /auth/me bootstrap
            if (!(options && options._silent) && error.message && error.message !== "Not authenticated") {
    ...
    me: () => API.call(`/auth/me`, { _silent: true }),
    createClearance: (d) =>
        API.call(`/clearances/create`, { method: "POST", body: JSON.stringify(d) }),
    getClearances: (filters = {}) => {
        const qs = Object.entries(filters).filter(([, v]) => v).map(([k, v]) => `${k}=${encodeURIComponent(v)}`).join("&");
        return API.call(`/clearances/list${qs ? `?${qs}` : ""}`);
    },
    getClearance: (cid) => API.call(`/clearances/${cid}`),
    processClearance: (cid, d) =>
        API.call(`/clearances/${cid}/process`, { method: "POST", body: JSON.stringify(d) }),
    getStats: () => API.call(`/stats`),
    getConstants: () => API.call("/constants"),
    getUsers: () => API.call(`/admin/users`),
    deleteUser: (target) => API.call(`/admin/users/${target}`, { method: "DELETE" }),
    getAuditLogs: (filters = {}) => {
        const qs = Object.entries(filters).filter(([, v]) => v).map(([k, v]) => `${k}=${encodeURIComponent(v)}`).join("&");
        return API.call(`/admin/audit-logs${qs ? `?${qs}` : ""}`);
    },
    getAuditLogActions: () => API.call(`/admin/audit-logs/actions`),
    getClearanceAuditTrail: (cid) => API.call(`/clearances/${cid}/audit-trail`),
};
...
async function applyFilters() {
    ...
    try {
        const result = await API.getClearances(filters);
...
async function viewClearance(id) {
    ...
    try {
        const result = await API.getClearance(id);
...
async function viewAuditTrail(clearanceId) {
    ...
    try {
        const { trail } = await API.getClearanceAuditTrail(clearanceId);
...
async function submitSignature(action) {
    ...
    try {
        await API.processClearance(clearanceId, { action, comments, signature_data: signatureData });
...
async function renderApp() {
    ...
    // Session-based: ask backend who we are
    try {
